...
import com.github.xphsc.util.URLUtil;
...
public class ClassScaner {
    ...
    public static Set<Class<?>> scanPackage(String packageName, Filter<Class<?>> classFilter) {
        ...
        Set classPaths = ClassUtil.getClassPaths(packageName);
        ...
        if(classes.isEmpty()) {
            String[] var9 = ClassUtil.getJavaClassPaths();
            String[] var10 = var9;
            int len$ = var9.length;
    ...
    private static void fillClass(String className, String packageName, Set<Class<?>> classes, Filter<Class<?>> classFilter) {
        if(className.startsWith(packageName)) {
            try {
                Class ex = Class.forName(className, false, ClassUtil.getClassLoader());
